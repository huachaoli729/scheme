;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;  This is the file generic.scm
;  Generic Operators for Polynomials
: @Huachao Li
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;; @Accepts two polynomial arguments and returns a polynomial which is their sum.

  (define (addPolys p1 p2) 
    (let ((v1 (variable p1))
          (v2 (variable p2)))
...
;;; @Accepts two polynomial arguments and returns a polynomial which is the first
;;;     polynomial minus the second one.
 
  (define (subPolys p1 p2)
...
;;; @Accepts two polynomial arguments and returns a polynomial which is the
;;;     product of the two polynomials.

  (define (mulPolys p1 p2)
    (let ((v1 (variable p1))
          (v2 (variable p2)))
      (if (same-variable? v1 v2)
          (make-poly v1
            ...
            (if (and order1 order2)
                (if (> order1 order2)
                    ...
                    (mul-poly 
                     p2 (make-poly v2 
                                   (list (make-term 0 (tag p1))))))
                (error "Don't know the order of the two variables -- ADD-POLYS"
                       (list p1 p2)))))))
...
;;; @Accepts a single polynomial as an argument and returns a polynomial which is its
;;; derivative (i.e., differentiates the polynomial).

  (define (diffPoly L)
    (if (empty-termlist? L)
        L
        (let ((t (first-term L)))
          (adjoin-term (make-term (* (order t) (coeff t)) (- (coeff t) 1))
                       (diffPoly (rest-terms L))))))
